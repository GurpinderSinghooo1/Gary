...
function syncData_web() {
  try {
    ...
    // Fetch data from all source tabs with error boundaries
    try {
      finalSignals = getFinalSignals();
    } catch (error) {
      errors.push(`Failed to fetch final signals: ${error.message}`);
    }
    
    try {
      technicalData = getTechnicalData();
    } catch (error) {
      errors.push(`Failed to fetch technical data: ${error.message}`);
    }
    
    try {
      fundamentals = getFundamentals();
    } catch (error) {
      errors.push(`Failed to fetch fundamentals: ${error.message}`);
    }
    
    try {
      marketSentiment = getMarketSentiment();
    } catch (error) {
      errors.push(`Failed to fetch market sentiment: ${error.message}`);
    }
    
    try {
      companyNames = getCompanyNameMap();
    } catch (error) {
      errors.push(`Failed to fetch company names: ${error.message}`);
    }
    ...
    if (!validateData(finalSignals, technicalData, fundamentals, marketSentiment)) {
      errors.push('Data validation failed - missing required fields or invalid data');
    }
    
    let enrichedData = [];
    if (finalSignals.length > 0) {
      // Merge data into enriched rows
      enrichedData = mergeSignalData(finalSignals, technicalData, fundamentals, marketSentiment, companyNames, dateString);
    }
    ...
    const validatedData = enrichedData.filter(row => {
      ...
      if (row.CurrentPrice && (isNaN(row.CurrentPrice) || row.CurrentPrice <= 0)) {
        errors.push(`Invalid current price for ${row.Ticker}: ${row.CurrentPrice}`);
        return false;
      }
      if (row.SellTarget && (isNaN(row.SellTarget) || row.SellTarget <= 0)) {
        errors.push(`Invalid sell target for ${row.Ticker}: ${row.SellTarget}`);
        return false;
      }
      
      // Validate confidence
      if (row.Confidence && (isNaN(row.Confidence) || row.Confidence < 0 || row.Confidence > 100)) {
        errors.push(`Invalid confidence for ${row.Ticker}: ${row.Confidence}`);
        return false;
      }
      
      // Validate timestamp
      if (row.Timestamp && !(row.Timestamp instanceof Date) && isNaN(new Date(row.Timestamp).getTime())) {
        errors.push(`Invalid timestamp for ${row.Ticker}: ${row.Timestamp}`);
        return false;
      }
    ...
    if (validatedData.length === 0) {
      errors.push('No valid signals after validation');
    }
    ...
    if (errors.length > 0) logSyncError(new Error(`Partial sync completed with errors: ${errors.join('; ')}`));
